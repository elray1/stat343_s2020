---
title: "Example for large sample credible intervals"
output: pdf_document
---
...
\begin{align*}
J(\hat{\theta}^{MLE}) &= -\frac{d^2}{d\theta^2} \ell(\theta | \mathbf{x}) \vert_{\theta = \hat{\theta}^{MLE}} \\
&= \frac{2 x_1 + x_2}{(1 - \hat{\theta}^{MLE})^2} + \frac{2 x_3 + x_2}{\left(\hat{\theta}^{MLE}\right)^2} \\
&= \frac{2 x_1 + x_2}{(1 - \frac{x_2 + 2 x_3}{2 n})^2} + \frac{2 x_3 + x_2}{\left(\frac{x_2 + 2x_3}{2 n}\right)^2}
\end{align*}

The approximate posterior distribution for $\Theta$ is therefore

$\Theta | X_1 = x_1, X_2 = x_2, X_3 = x_3 \sim \text{Normal}\left[ \frac{x_2 + 2 x_3}{2 n}, \left\{ \frac{2 x_1 + x_2}{(1 - \frac{x_2 + 2 x_3}{2 n})^2} + \frac{2 x_3 + x_2}{\left(\frac{x_2 + 2x_3}{2 n}\right)^2} \right\}^{-1} \right]$
