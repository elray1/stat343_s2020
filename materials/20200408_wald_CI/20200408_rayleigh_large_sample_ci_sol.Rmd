...
#### 1. Find an expression for the observed Fisher information as a function of $\theta^*$ (not much work to do here...).

$J(\theta^*) = -\frac{n}{(\theta^*)^2} + \frac{1}{(\theta^*)^3} \sum_{i=1}^n x_i^2$

\vspace{3cm}

#### 2. Find an expression for the Fisher information as a function of $\theta^*$.  When you evaluate at $\theta^*$, it will be most convenient if you first evaluate the expected value and then plug in $\theta^*$ for $\theta$.  You can do this because the Rayleigh distribution pdf satisfies the regularity conditions necessary interchange differentiation and integration.

\begin{align*}
I(\theta^*) &= E\left[-\frac{n}{(\theta)^2} + \frac{1}{(\theta)^3} \sum_{i=1}^n X_i^2 \right] \vert_{\theta = \theta^*} \\
&= \left[ -\frac{n}{(\theta)^2} + \frac{1}{(\theta)^3} \sum_{i=1}^n E(X_i^2) \right] \vert_{\theta = \theta^*} \\
&= \left[ -\frac{n}{(\theta)^2} + \frac{1}{(\theta)^3} n 2 \theta \right] \vert_{\theta = \theta^*} \\
