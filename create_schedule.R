...
start_day <- "1-21"
end_day <- "4-28"
start_finals_day <- "5-1"
end_finals_day <- "5-5"
start_midsemester_break <- "3-14"
end_midsemester_break <- "3-22"
community_day <- "4-14"
...
community_day_date <- lubridate::mdy(paste0(community_day, "-", year))
...
while(week_mon <= end_date) {
  week_day_text <- paste(
    sapply(class_days, function(class_day) {
      ...
      } else if(class_date == community_day_date) {
        return(paste0("\n#### ", format(class_date, "%a, %b %d"),
